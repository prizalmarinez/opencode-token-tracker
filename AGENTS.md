...
- `server/index.mjs` — the HTTP adapter. Resolves the db path (`?db=` → `OPENCODE_DB` → default), checks CORS **once** at the top of the request, routes `/api/*` to the query module, serializes JSON. No SQL here. `MAX_LIMIT` here must stay ≥ `PAGE_SIZE` in `src/lib/api.ts`.
...
- `@/*` aliases `src/*` (vite + tsconfig). No router library — pathname routing in `src/App.tsx`: `/usage`, `/settings`, `/project/<encodeURIComponent(key)>`. Project rows link to `/project/...`; `parseProject()` decodes the key.
- **Data fetching is one primitive**: `src/lib/use-query.ts` exports `useQuery(fetcher, deps)` (refetch when deps change, cancel on unmount, per-query error/loading) and `useAnyQueryLoading()` (drives the nav refresh spinner). `App.tsx` is the composition root: it owns `dbPath` and a `refreshKey` (the nav refresh bumps it), and each page composes its own queries through `useQuery` — `Usage` (`getSummary` + `getSessions`), `SettingsPage` (`getStatus`), `ProjectPage` (scoped `getSummary` + `getProjectSessions` for the neural net). Every prop a fetcher reads must be in its `deps`. Aggregation is server-side; never re-GROUP BY client-side.
